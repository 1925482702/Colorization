...
class TestModel(BaseModel):
    """ 这个TestModel可以用于只生成一个方向的CycleGAN结果。
    该模型将自动设置'--dataset_mode single'，只加载一个集合的图像。
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        参数:
            parser          -- 原始选项解析器
            is_train (bool) -- 是否处于训练阶段或测试阶段。您可以使用此标志添加特定于训练或测试的选项。
        ...
        parser.add_argument('--model_suffix', type=str, default='', help='在checkpoints_dir中，[epoch]_net_G[model_suffix].pth将作为生成器加载。')
